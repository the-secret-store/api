...
/**
 * Generates a random unique(almost) name
 * @returns {string} A random name of form LEFT-RIGHT-Hex6
 */
export default function generateRandomName() {
	const randomLeft = LEFT[Math.floor(Math.random() * LEFT.length)];
	const randomRight = RIGHT[Math.floor(Math.random() * RIGHT.length)];
	const randomHex = generateRandomHex(6);

	return `${randomLeft}-${randomRight}-${randomHex}`;
}
